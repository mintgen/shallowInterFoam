...
#include "sifKFvPatchScalarField.H"
...
Foam::sifKFvPatchScalarField::
sifKFvPatchScalarField
(
    const fvPatch& p,
...
sifKFvPatchScalarField
(
    const sifKFvPatchScalarField& ptf,
    const fvPatch& p,
    const DimensionedField<scalar, volMesh>& iF,
...
sifKFvPatchScalarField
(
    const sifKFvPatchScalarField& wtcsf,
    const DimensionedField<scalar, volMesh>& iF
)
:
...
Foam::sifKFvPatchScalarField::baseArea(const polyMesh& localMesh, const fvPatch& nbrPatch, const mapDistribute& distMap) const
{
...
Foam::vectorField 
Foam::sifKFvPatchScalarField::flowdepth3d(const polyMesh& nbrMesh, const fvPatch& nbrPatch, const mapDistribute& localMap, const scalarField& baseAreaField) const
{
...
Foam::sifKFvPatchScalarField::wallShearStresses(const polyMesh& localMesh, const fvPatch& nbrPatch, const mapDistribute& localMap, const mapDistribute& distMap) const
{
...
void Foam::sifKFvPatchScalarField::updateCoeffs()
{
...
//        uStar[i][2] = pow(mag(wssField[i][2]),0.5) * sign(velNbrIntField[i][2]);
    ...
    forAll(alpha1NbrIntField, i)
    {
	  ...
	  this->valueFraction()[i] = 0.0;
        ...
        // In case of inflow, set profile for k
        else
        {
	  ...
	  //this->refValue()[i] = 4. * exp(-1.7*zFaceRel);
	  //	  this->refValue()[i] = 4. * pow(mag(uStar[face2d]),2.) * exp(-1.7*zFaceRel);
	  this->refValue()[i] = max(4. * pow(mag(uStar[face2d]),2.) * exp(-1.7*zFaceRel),0.001);
	  ...
	  if(this->refValue()[i] == 0.0)
	    {
	      this->valueFraction()[i] = 0.0 + SMALL;
	    }
	  */
        }
...
void Foam::sifKFvPatchScalarField::write
(
    Ostream& os
) const
{
